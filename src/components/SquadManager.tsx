...
import { SquadIcon, PlusIcon, EditIcon, DeleteIcon, CloseIcon, ImportIcon } from "./icons";
    ...
    <section>
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setPasting((p) => !p); setAdding(false); }} className="btn-ghost text-xs">
            <ImportIcon className="h-3.5 w-3.5" /> Pegar plantilla
          </button>
          <button onClick={() => { setAdding((a) => !a); setPasting(false); }} className="btn-ghost text-xs">
      ...
      {pasting && (
        <div className="card mb-4 p-4">
          <h3 className="mb-3 font-semibold text-ink">Pegar plantilla completa</h3>
          <BulkAddPlayers teamId={teamId} onDone={() => setPasting(false)} />
        </div>
      )}
