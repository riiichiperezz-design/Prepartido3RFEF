  ...
  return (
    <Link href={`/teams/${team.id}`} className="card card-hover block p-4">
      <div className="flex items-start gap-3">
        <Avatar name={team.shortName ?? team.name} src={team.crestUrl} size="lg" square variant="team" />
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <h3 className="truncate font-semibold tracking-tight text-ink">{team.name}</h3>
            {team.currentPosition && (
              <span className="chip bg-gray-100 text-ink-muted">{team.currentPosition}.º</span>
            )}
          </div>
